...
static void SearchContact(List<int> ids, Dictionary<int, string> names, Dictionary<int, string> lastnames,
    ...
    Dictionary<int, int> ages, Dictionary<int, bool> bestFriends)
{
    Console.WriteLine("Digite el nombre o ID del contacto que desea buscar:");
    string input = Console.ReadLine();

    int id;
    bool found = false;

    if (int.TryParse(input, out id)) // Buscar por ID
    {
        if (ids.Contains(id))
        {
            PrintContact(id, names, lastnames, addresses, telephones, emails, ages, bestFriends);
            found = true;
        }
    }
    else // Buscar por nombre
    {
        foreach (var kvp in names)
        {
            ...
            {
                PrintContact(kvp.Key, names, lastnames, addresses, telephones, emails, ages, bestFriends);
                found = true;
            }
        }
    }
...
static void PrintContact(int id, Dictionary<int, string> names, Dictionary<int, string> lastnames,
    Dictionary<int, string> addresses, Dictionary<int, string> telephones, Dictionary<int, string> emails,
    Dictionary<int, int> ages, Dictionary<int, bool> bestFriends)
{
    string isBestFriendStr = bestFriends[id] ? "Si" : "No";
    Console.WriteLine($"{id} - {names[id]} {lastnames[id]}, {addresses[id]}, Tel: {telephones[id]}, Email: {emails[id]}, Edad: {ages[id]}, Mejor amigo: {isBestFriendStr}");
}
